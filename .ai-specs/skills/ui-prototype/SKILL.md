...
*   **全量设计模式 (Full Design)**:
    *   `specs/1_产品概述.md` (核心输入，用于提取功能模块清单和核心价值)
...
*   **问题**: 在仅依赖 `specs/1_产品概述.md` 生成概念稿时，容易遗漏关键功能点。
*   **解决方案**:
    *   **生成前自检**: AI 必须先从 `specs/1_产品概述.md` 中提取该模块的 **"核心价值" (Core Value)** 和 **"用户故事" (User Stories)**。
