...
## 目标
你的目标是解决“先做什么，后做什么”的问题。基于《产品概述》中的核心板块，分析模块间的依赖关系，制定一份合理的**开发路线图 (Development Roadmap)**，即 `6_开发路线图.md`。
...
5.  **最终交付**：保存到 `.ai-specs/steering/6_开发路线图.md`。
...
- [ ] **Phase 1.0: 全局 UI 原型 (Global UI Prototyping)**
    *   **模块**: 全站
    *   **操作**: 调用 `ui-prototype` (全量设计模式)。
    *   **产出**: `docs/product_prototypes/` 下的所有核心页面 HTML。
    *   **理由**: 视觉先行，消除开发阶段的“不确定性”。
