...
## 你的任务
基于技术栈 (`rules/tech-stack.md`) 和核心业务 (`rules/product-overview.md`)，生成具体的项目目录结构。

## 工作流程
1.  **读取上下文**：
    *   读取 `rules/tech-stack.md`：确定是用 Next.js 的路由结构，还是 Go 的 clean architecture。
    *   读取 `rules/product-overview.md`：提取“核心板块”（如 Auth, User, Order），将它们映射到模块目录中。
...
- **最终交付**：当文档内容被用户确认后，请将其保存到 `rules/project-structure.md`。
