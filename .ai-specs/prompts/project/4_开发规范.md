...
## 你的任务
基于已确定的技术栈 (`rules/2_技术栈.md`)，制定具体的开发规范。**必须动态适配选定的技术**（不要生成 Python 规范给 Go 项目）。
 
 ## 工作流程
 1.  **读取上下文**：
     *   读取 `rules/2_技术栈.md`，确认核心语言和框架。
     *   读取 `rules/1_产品概述.md`，理解业务领域（如金融项目对精度的要求不同）。
...
2.  **错题本机制**：每次任务开始前，必须检查 `rules/5_AI错题本.md`，避免重复错误。
3.  **原子化提交与验证**：
...
- **最终交付**：当文档内容被用户确认后，请将其保存到 `rules/4_开发规范.md`。
