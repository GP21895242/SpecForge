...
## 你的任务
基于已确定的技术栈 (`rules/tech-stack.md`)，制定具体的开发规范。**必须动态适配选定的技术**（不要生成 Python 规范给 Go 项目）。

## 工作流程
1.  **读取上下文**：
    *   读取 `rules/tech-stack.md`，确认核心语言和框架。
    *   读取 `rules/product-overview.md`，理解业务领域（如金融项目对精度的要求不同）。
    *   **全量规则扫描**：必须扫描 `.trae/rules` 或 `.ai-specs/rules` 下的所有文档，确保不遗漏任何约束。
...
2.  **错题本机制**：每次任务开始前，检查 `rules/ai-error-log.md`，避免重复错误。
3.  **原子化提交与验证**：
...
- **最终交付**：当文档内容被用户确认后，请将其保存到 `rules/dev-standards.md`。
