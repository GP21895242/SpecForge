...
2. **执行阶段验证检查点**：读取任务规划文档中 `4.2 阶段验证检查点` 表格，按照该阶段对应的验证动作逐项执行，确认是否达到通过标准
3. **向用户汇报阶段完成情况**（简要摘要，不需要用户确认即可继续下一阶段）
...
读取任务规划文档中 `4.4 最终验证` 和 `4.3 验收标准逐项验证`，执行以下验证：

- 运行全量测试套件（覆盖率 > 80%）
- 按照 `4.3 验收标准逐项验证` 表格，逐条 AC 验证（确认验证方式、关联任务、结果）
- 代码规范检查（Linter + 类型检查）
- 性能测试（响应时间、并发量，如适用）
- 安全测试（权限校验、数据校验，如适用）
- 记录所有验证结果
