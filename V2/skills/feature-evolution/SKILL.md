...
description: "功能迭代变更管理。对已完成开发闭环的功能进行增量修改、扩展或优化，生成变更影响分析和增量任务计划。"
---
...
> **⚠️ 停止符 (Stop Sign)**：交付物仅限上述 Markdown 文档。保存完任务计划后，**必须立即停止，绝对不要越界去修改或编写任何业务代码**。
...
*   **回归意识**：对每个变更，必须评估是否会破坏已有功能，并在任务中包含回归测试。
...
*   **增量任务编号**：增量任务编号从原任务规划的最后一个任务编号之后继续（如原任务到 Task-11，则增量从 Task-12 开始）。
*   **回归测试必选**：每个增量任务计划必须包含一个回归测试任务，验证变更未破坏已有功能。
...
如果用户要求你直接写代码，或者你准备在输出变更任务后继续写代码，必须**立即停止**，并回复：
